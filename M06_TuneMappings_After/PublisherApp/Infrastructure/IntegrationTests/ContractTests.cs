...
public class ContractTests
{
    ...
    public void NewContractHasVersionWithSpecDefaults()
    {
        ...
        Assert.AreEqual(GetDefaultSpecs(), contractFromDB.Versions.First().Specs);
    }

    private  SpecificationSet GetDefaultSpecs()
    {
        //read from json file
...
[TestMethod]
    ...
    public void EFCore7_HasRevisedSpecSetIsFalseForNewContractNewVersion()
    {
        ...
        var value = _context.Database
               .SqlQuery<bool>($"SELECT [_hasRevisedSpecSet] as [Value] FROM [ContractVersion]")
               .FirstOrDefault();
        Assert.AreEqual(false, value);
    }
    ...
    public void FullContractAggregateValuesPersistedandRetrieved()
    {
        ...
        _context.SaveChanges();
        _context.ChangeTracker.Clear();
        var contractFromDB = _context.Contracts //edited this or record spec set change
            .Include(c => c.Versions).FirstOrDefault();
