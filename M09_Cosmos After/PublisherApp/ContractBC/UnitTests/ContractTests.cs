...
public class ContractTests
{
    ...
    {
        var contract = new Contract(DateOnly.FromDateTime(DateTime.Now), _unsignedAuthors, "booktitle");
    ...
    [TestMethod]
    public void AddedCommentsStartsWithTodaysDate()
    {
        _contract.AddNewComments("hello");
        Assert.AreEqual(DateTime.Today.ToShortDateString(),
            _contract.Comments.First().Substring(0, 10).TrimEnd());
    }
    [TestMethod]
    public void AddedCommentsContainsComment()
    {
        _contract.AddNewComments("hello");
        Assert.IsTrue(_contract.Comments.First().Contains("hello"));
    }
}
